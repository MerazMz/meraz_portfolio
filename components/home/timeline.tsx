...
const TimelineSection = ({ isDesktop }: IDesktop) => {
  ...
  const addText = (
    ...
    const titleSizeClass =
      size === ItemSize.LARGE
        ? "text-5xl font-bold font-outfit text-white"
        : "text-xl font-semibold font-outfit text-white";
    const logoString = image
      ? `<img src='${image}' class='h-6 w-auto mb-2 object-contain' loading='lazy' alt='${image}' />`
      : "";
    const subtitleString = subtitle
      ? `<p class='text-sm mt-1 font-outfit leading-relaxed' style='color:#888'>${subtitle}</p>`
      : "";
  ...
  const renderSlides = (): React.ReactNode => (
    <div
      className="max-w-full h-96 overflow-hidden rounded-3xl"
      ref={screenContainer}
      style={{
        background: "#171717",
  ...
  const renderSectionTitle = (): React.ReactNode => (
    <div className="flex flex-col">
      <p className="section-title-sm seq font-outfit">MILESTONES</p>
      <h1 className="section-heading seq mt-2 font-outfit">Timeline</h1>
      <h2 className="text-2xl md:max-w-2xl w-full seq mt-2 font-outfit">
        A quick recap of{" "}
        <span className="font-nyght italic text-[#676767]">proud moments.</span>
      </h2>
    </div>
  );
